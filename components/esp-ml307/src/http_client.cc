...
HttpClient::~HttpClient() {
    // 先唤醒可能堵在 OnTcpData 里的接收任务，再 join。tcp_ 最后析构是第二道防线。
    connected_ = false;
    eof_ = true;
...
bool HttpClient::ParseUrl(const std::string& url) {
    ...
    // Paths can carry API keys or signed query parameters; keep them out of
    // logs just like Authorization headers.
    ESP_LOGD(TAG, "Parsed URL: protocol=%s, host=%s, port=%d",
...
std::string HttpClient::BuildHttpRequest() {
    ...
    // 结束头部
    request << "\r\n";
    // Do not dump the assembled headers here: callers may include an
    // Authorization bearer token, and debug logging must not turn it into a
    // serial-console credential leak.
    ESP_LOGD(TAG, "HTTP request prepared: method=%s host=%s", method_.c_str(), host_.c_str());
...
bool HttpClient::Open(const std::string& method, const std::string& url) {
    ...
    if (can_reuse) {
        ...
        // 旧连接即使已被对端关掉，也要 join 接收任务，再换新的 Tcp 对象
        if (tcp_) {
...
void HttpClient::Close() {
    ...
    cv_.notify_all();
    if (tcp_) {
        // 必须 join：connected_ 为 false 只表示对端已关，接收任务可能还在回调里
        tcp_->Disconnect();
