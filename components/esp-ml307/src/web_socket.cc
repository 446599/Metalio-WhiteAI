...
bool WebSocket::Connect(const char* uri) {
    ...
    // A WebSocket path may include a signed query string; never print it.
    ESP_LOGD(TAG, "Connecting to %s://%s:%s", protocol.c_str(), host.c_str(), port.c_str());
...
void WebSocket::OnTcpData(const std::string& data) {
    ...
    // RX assembly state is owned by this connection. A reconnect must not
    // inherit a partial message or opcode from the previous socket.
    ...
    while (buffer_offset < buffer_size) {
        ...
        if (payload_length == 126) {
            ...
            payload_length = (static_cast<uint8_t>(buffer[buffer_offset + 2]) << 8) | static_cast<uint8_t>(buffer[buffer_offset + 3]);
            header_length += 2;
        } else if (payload_length == 127) {
            if (buffer_size - buffer_offset < 10) break; // 需要更多数据
            payload_length = 0;
            for (int i = 0; i < 8; ++i) {
                payload_length = (payload_length << 8) | static_cast<uint8_t>(buffer[buffer_offset + 2 + i]);
            }
            header_length += 8;
        }
        ...
        switch (opcode) {
            ...
            case 0x2: // 二进制帧
                ...
                if (fin) {
                    if (on_data_) {
                        on_data_(current_message_.data(), current_message_.size(), rx_binary_);
                    }
                    current_message_.clear();
                    rx_fragmented_ = false;
                }
