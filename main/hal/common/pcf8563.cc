...
bool Pcf8563::ApplyRtcToSystem() {
    ...
    if (!valid) {
        ESP_LOGW(TAG, "RTC lost power; wait for network time before scheduling reminders");
        return false;
    }

    if (rtc_tm.tm_year < 124 || rtc_tm.tm_year > 199 || rtc_tm.tm_mon < 0 || rtc_tm.tm_mon > 11 ||
        rtc_tm.tm_mday < 1 || rtc_tm.tm_mday > 31 || rtc_tm.tm_hour > 23 ||
        rtc_tm.tm_min > 59 || rtc_tm.tm_sec > 59) return false;
