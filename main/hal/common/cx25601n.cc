...
int get_chrg_stat_count(uint8_t expected)
{
    int count = 0;
    for (int i = 0; i < 5; i++) {
        uint8_t stat = 0;
        if (cx25601n_get_chrg_stat(&stat) == ESP_OK && stat == expected) {
            count++;
        }
    }
    return count;
...
void charger_vreg_task(void *arg)
{
    ...
    while (true) {
        ...
        uint32_t vreg = s_vreg_vol_mv;
        if (vreg < CX25601N_VREG_MIN_MV || vreg > CX25601N_VREG_MAX_MV) {
            continue;
        }

        int32_t vbat = battery_get_bat_voltage();
        int32_t ibat = battery_get_bat_current() / 10;
        //int32_t temp = battery_get_bat_temperature();
        ESP_LOGI(TAG, "vbat=%ldmV ibat=%ld vreg=%lumV now=%lumV",
                 static_cast<long>(vbat), static_cast<long>(ibat),
                 static_cast<unsigned long>(vreg), static_cast<unsigned long>(s_vreg_now_mv));

        if (get_chrg_stat_count(CX25601N_CHG_STAT_CV) >= 2) {
            if (vbat < static_cast<int32_t>(vreg)) {
                int32_t vreg_diff = static_cast<int32_t>(vreg) - vbat;
                vreg_diff = (vreg_diff / static_cast<int32_t>(CX25601N_VREG_STEP_MV)) *
                            static_cast<int32_t>(CX25601N_VREG_STEP_MV);

                if (ibat >= 1000000) {
                    vreg_diff -= 20;
                    if (vreg_diff >= static_cast<int32_t>(CX25601N_VREG_STEP_MV)) {
                        s_vreg_now_mv += static_cast<uint32_t>(vreg_diff);
                    }
                } else {
                    if (vreg_diff >= static_cast<int32_t>(CX25601N_VREG_STEP_MV)) {
                        s_vreg_now_mv += static_cast<uint32_t>(vreg_diff);
                ...
                uint32_t vreg_max = vreg + 100;
                if (vreg_max > CX25601N_VREG_MAX_MV) {
                    vreg_max = CX25601N_VREG_MAX_MV;
                }
                if (s_vreg_now_mv > vreg_max) {
                    s_vreg_now_mv = vreg_max;
                }
                if (s_vreg_now_mv < CX25601N_VREG_MIN_MV) {
                    s_vreg_now_mv = CX25601N_VREG_MIN_MV;
                }
                set_vreg_hw_mv(s_vreg_now_mv);
            }

            if (vbat > static_cast<int32_t>(vreg + 5) &&
                s_vreg_now_mv >= CX25601N_VREG_MIN_MV + CX25601N_VREG_STEP_MV) {
                s_vreg_now_mv -= CX25601N_VREG_STEP_MV;
                set_vreg_hw_mv(s_vreg_now_mv);
            }
        } else if (get_chrg_stat_count(CX25601N_CHG_STAT_CC) >= 3) {
            if (vbat >= static_cast<int32_t>(vreg)) {
                s_vreg_count++;
                if (s_vreg_count > 10) {
                    s_vreg_count = 10;
                }
                int32_t target_vreg = static_cast<int32_t>(vreg) -
                                      static_cast<int32_t>(CX25601N_VREG_STEP_MV * s_vreg_count);
                if (target_vreg < CX25601N_VREG_MIN_MV) {
                    target_vreg = CX25601N_VREG_MIN_MV;
                }
                s_vreg_now_mv = static_cast<uint32_t>(target_vreg);
                set_vreg_hw_mv(s_vreg_now_mv);
            }
        }

        uint8_t stat = 0;
        if (cx25601n_get_chrg_stat(&stat) == ESP_OK && stat == CX25601N_CHG_STAT_NOT) {
            s_vreg_count = 0;
        }
    }
}
