...
BTAudioCodecDuplex::BTAudioCodecDuplex(int input_sample_rate, int output_sample_rate, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din)
{
    ...
    // 硬件测试不做 AEC：单声道帧 API，与 Write 语义一致
...
{
    // samples = 单声道帧数（与 Write 一致）；I2S 仍为立体声，读 samples*2 个 slot 后取左声道
    size_t bytes_read = 0;
    std::vector<int32_t> bit32_buffer(static_cast<size_t>(samples) * 2);
    const esp_err_t err =
        i2s_channel_read(rx_handle_, bit32_buffer.data(),
                         static_cast<size_t>(samples) * 2 * sizeof(int32_t), &bytes_read,
                         pdMS_TO_TICKS(1000));
    ...
    const int frames = static_cast<int>(bytes_read / (2 * sizeof(int32_t)));
    for (int i = 0; i < frames; i++) {
        int32_t value = bit32_buffer[static_cast<size_t>(i) * 2] >> 12;
