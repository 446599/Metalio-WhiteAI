...
#define esp_partition_munmap(handle) ((void)0)
#endif
#ifdef __cplusplus
...
static bool header_ok(const fontpack_header_t* hdr, size_t blob_size) {
    ...
    if (hdr->total_items == 0 || hdr->total_items > 200000 || hdr->index_offset < sizeof(*hdr)) return false;
...
int font_loader_init_memory(const uint8_t* data, size_t size) {
    if (!data || size < sizeof(fontpack_header_t)) return FONT_LOADER_ERR_FORMAT;
    if (s_ctx.backend != FONT_BACKEND_NONE) return FONT_LOADER_ERR_STATE;
    fontpack_header_t header;
    memcpy(&header, data, sizeof(header));
    if (!header_ok(&header, size)) return FONT_LOADER_ERR_FORMAT;
    // Validate every range, key and bitmap length once; rendering can then do
    // bounded binary searches even if a font partition is damaged or replaced.
    uint64_t previous = 0;
    for (uint32_t i = 0; i < header.total_items; ++i) {
        fontpack_index_entry_t entry;
        memcpy(&entry, data + header.index_offset + i * sizeof(entry), sizeof(entry));
        if ((i && entry.key <= previous) || entry.data_offset < header.data_offset ||
            entry.data_size < sizeof(fontpack_glyph_meta_t) ||
            (uint64_t)entry.data_offset + entry.data_size > size) return FONT_LOADER_ERR_FORMAT;
        previous = entry.key;
        fontpack_glyph_meta_t meta;
        memcpy(&meta, data + entry.data_offset, sizeof(meta));
        const unsigned bpp = entry.key & 0xffff;
        if ((bpp != 1 && bpp != 2) || meta.width > 256 || meta.height > 256 || meta.advance > 256 ||
            ((uint32_t)meta.width * bpp + 7) / 8 * meta.height != entry.data_size - sizeof(meta))
            return FONT_LOADER_ERR_FORMAT;
    }
    s_ctx.backend = FONT_BACKEND_FLASH;
    s_ctx.mmap_ptr = data;
    s_ctx.mmap_size = size;
    s_ctx.total_items = header.total_items;
    s_ctx.index_offset = (uint32_t)header.index_offset;
    s_ctx.data_offset = (uint32_t)header.data_offset;
    return FONT_LOADER_OK;
}

int font_loader_init_flash(const char* partition_label) {
#ifdef FONTPACK_HOST_TEST
    ...
    const int validated = font_loader_init_memory(base, part->size);
    if (validated != FONT_LOADER_OK) {
        esp_partition_munmap(handle);
        s_ctx.last_err = validated;
        return validated;
    }
